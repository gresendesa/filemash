...
class FileMasher:
	...
	def get_root_dir(self):
		...
		if len(self.file_name) > 1 and len(os.path.dirname(self.file_name)) > 0:
			root_dir = os.path.dirname(self.file_name) + '/'
		return root_dir
	...
	def get_branch_path(self, deep = None):
		...
		for i in range(0, deep):
			if len(os.path.dirname(self.DEPENDENCY_BRANCH[i])) > 0:
				path += os.path.dirname(self.DEPENDENCY_BRANCH[i]) + '/'
		return path
	...
	def append_branch_path(self, path, just_basename = True):
		...
		"""
		if just_basename:
			return os.path.normpath(self.get_branch_path() + os.path.basename(path))
		else:
			return os.path.normpath(self.get_branch_path() + path)
	...
	def get_parent_file_path(self, connection):
		...
		if path_pos_connection > 0:
			return os.path.normpath(self.get_branch_path(path_pos_connection - 1) + self.get_basename(self.DEPENDENCY_BRANCH[path_pos_connection - 1]))
		else:
	...
	def build_file(self, file_path):
		...
		"""
		with open(self.append_branch_path(file_path), 'r') as file:
			built_file = ''
			line_counter = 0
			for line in file:
				line_counter += 1
...
f = FileMasher(file_name="sample/animals_and_fruits.txt")
f.mash()